...
const gameCurrencies = [
    {name: "Tockens", value: 0},
    {name: "Time Crystals", value: 0}
];
...
const GameProvider = ({ children }) => {
    ...
    const addCurrency = (currencyName, amount) => {
        const getCurrency = JSON.parse(localStorage.getItem("gameCurrencies"));
        const currentCurrency = getCurrency.find(statistics => statistics.name === currencyName).value;
    ...
    const updateCurrencyInLocalStorage = (newCurrency) => {
        ...
        // Find the index of the currency to update
        const currencyIndex = getCurrency.findIndex(currency => currency.name === newCurrency.name);
    
        if (currencyIndex !== -1) {
            // Update the existing currency
            getCurrency[currencyIndex].value = newCurrency.value;
        } else {
            // If the currency doesn't exist, add it to the array
            getCurrency.push(newCurrency);
        }
    
        // Save the updated currencies back to localStorage
        localStorage.setItem("gameCurrencies", JSON.stringify(getCurrency));
    };
    ...
    return (
        <GameContext.Provider value={{ onBreakClass, checkGameStats, generateCurrency, validateClick }}>
            {children}
